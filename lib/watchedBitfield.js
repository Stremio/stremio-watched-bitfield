...
watchedBitfield.constructAndResize = function(serialized, videoIds) {
	...
	var offset = (length-1) - lastVideoIdx

	if (length != videoIds.length) {
		length = videoIds.length

		var resizedBuf = new watchedBitfield(new BitField8(length), videoIds)
	...
	var buf = BitField8.fromPacked(atob(components[2]), length)
